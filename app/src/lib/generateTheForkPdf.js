// ── TheFork PDF Generation ─────────────────────────────────────────
// Generates a branded PDF from a report + interpretations.
// Uses html2canvas + jsPDF (same strategy as generatePdf.js)
// but renders TheForkPdfRenderer instead of PdfReportRenderer.
...
async function captureBlock(el) {
  const canvas = await html2canvas(el, {
    scale: 2,
    useCORS: true,
    backgroundColor: theForkTokens.color.paper,
    logging: false,
    allowTaint: true,
...
/**
 * Generate and download a TheFork-branded PDF from a report + interpretations.
 *
 * @param {object} report — The report object ({ id, title, subtitle, kpis, sections, ... })
...
export async function generateTheForkPdf(report, interpretations = {}) {
  ...
  container.style.cssText = `width: 800px; background: ${theForkTokens.color.paper};`;
  wrapper.appendChild(container);
  document.body.appendChild(wrapper);

  // ── 2. Render TheFork branded React component ──
  const root = createRoot(container);
  root.render(createElement(TheForkPdfRenderer, { report, interpretations }));
  ...
  try {
    ...
    const containerPxW = 800;
    const scaleFactor = CONTENT_W / containerPxW;
    ...
    for (let i = 0; i < captures.length; i++) {
      ...
      const blockH_mm = cap.h * scaleFactor;

      // New page if block doesn't fit and we're not at top
      ...
      // Handle oversized blocks (taller than one page)
      if (blockH_mm > CONTENT_H) {
        const imgW_px = cap.w * 2;
        let srcY = 0;
    ...
    // ── 6. Add page numbers + footer on all pages ──
    ...
    for (let p = 1; p <= totalPages; p++) {
      ...
      pdf.setFontSize(7);
      pdf.setTextColor(115, 115, 115); // ink.500

      // Left: branding
      pdf.text("The Fork — Rapport interne — Pilot Pilotage Financier", MARGIN.left, PDF_H - 8);
      // Center: page number
      pdf.text(`${p} / ${totalPages}`, PDF_W / 2, PDF_H - 8, { align: "center" });
      // Right: date
      pdf.text(dateStr, PDF_W - MARGIN.right, PDF_H - 8, { align: "right" });
    }
